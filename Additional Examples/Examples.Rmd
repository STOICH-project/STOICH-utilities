---
title: "STOICH-Utilities Examples"
...
output: pdf_document
vignette: >
  %\VignetteIndexEntry{STOICH-Utilities Examples}
...
library(sf)
library(leaflet)
library(webshot) # for snapshots of leaflet plots
library(mapview) # for snapshots of leaflet plots
...
basePath <- file.path(path.expand("~"), "data", "stoichdb", "currentDB", "STOICH_Beta_Release_2025-11-03")
stoichData <- stoichUtilities::loadSTOICH(dataPath=basePath)

# OneDrive <- file.path(path.expand("~"), "..", "OneDrive - University of Nebraska-Lincoln", "STOICH", "Presentations", "Beta Users")
...
DataExplorer::create_report(stoichTable, output_format=rmarkdown::html_fragment(), output_dir = path.expand("~"), output_file = "DataExplorer.html")
```
```{r DataExplorer Link, results='asis', echo=FALSE}
cat(paste0("<a href=\"", path.expand("~"), "/DataExplorer.html\">DataExplorer report output</a>"))
```
...
myPlot <- myPlot |>
  ...
  leaflet::addAwesomeMarkers(data = data.frame(stoichTable |> filter(TaxonomicOrder == "hypnales")), lng = ~Longitude, lat = ~Latitude, icon=blueIcon) |>
  leaflet::addAwesomeMarkers(data = data.frame(stoichTable |> filter(TaxonomicOrder == "marchantiales")), lng = ~Longitude, lat = ~Latitude, icon=greenIcon) |>
  leaflet::addAwesomeMarkers(data = data.frame(stoichTable |> filter(TaxonomicOrder == "polygonales")), lng = ~Longitude, lat = ~Latitude, icon=orangeIcon)
```
```{r Mapping_Example_fake_display, eval=FALSE, echo=TRUE}
# Display the plot.
myPlot
```
```{r Mapping_Example_display, eval=TRUE, echo=FALSE, out.width=3.5}
# Not sure why this doesn't generate an actual image. This needs to be generated separately so the image can be included below.
mapshot(myPlot, file = file.path(path.expand("~"), "temp_r_image.png"),
        remove_controls = c("homeButton", "layersControl"))
```
![](C:/Users/cpetersen4/Documents/temp_r_image.png){out.width=3.5}
...
```{r Mapping_Example2_code, eval=FALSE, echo=TRUE}
...
```
```{r Mapping_Example2_plot, eval=TRUE, echo=FALSE}
maxYear <- (stoichTable |>
  group_by(SampleYear.SampleEvent) |>
  summarise(N=n()) |>
  filter(N==max(N)))$SampleYear.SampleEvent

stoichTable <- filter(stoichTable, maxYear==SampleYear.SampleEvent)

# Map the state borders
ggplot(data = state_info, mapping = aes(x=long, y=lat, group=group)) +
  geom_polygon(color = "black", fill = "white") + 
  coord_quickmap() +
  theme_void() +
  # Add the Seston data to the map of state borders and set the color using the C:N ratio.
