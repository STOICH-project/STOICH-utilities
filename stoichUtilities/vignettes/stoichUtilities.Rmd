...
dbVersionList <- list.files(stoichData)
dbCurrentVersion <- keep(dbVersionList, ~str_detect(.x, paste0(max(str_extract(dbVersionList, "[0-9]{4}-[0-9]{2}-[0-9]{2}"), na.rm=TRUE), "(?!.*\\.zip$)")))
stoichData <- stoichUtilities::loadSTOICH(dataPath=file.path(stoichData, dbCurrentVersion))
...
## Data Structure
![Data structure and relationship for STOICH data tables.](~/GitHub/STOICHDatabase/docs/data_model_narrow.svg){#id .class width=70% height=70%}
